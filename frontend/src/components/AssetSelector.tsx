 ...
 * bounded edit-distance typo tolerance. Results are ranked by:
 *   - Exact symbol match (highest priority)
 *   - Symbol prefix match
 *   - Symbol substring match
 *   - Name prefix and substring matches
 *   - Typo-tolerant symbol/name matches
 */

import React, { useState, useCallback, useMemo, useRef, useEffect } from 'react';
import { getAssetSearchScore } from './assetSelectorSearch';
...
export function AssetSelector({
  ...
  const { filteredAssets, groups }: { filteredAssets: Asset[]; groups: AssetGroup[] } = useMemo(() => {
    ...
    if (searchQuery) {
      filtered = filtered
        .map(asset => ({ asset, score: getAssetSearchScore(asset, searchQuery) }))
